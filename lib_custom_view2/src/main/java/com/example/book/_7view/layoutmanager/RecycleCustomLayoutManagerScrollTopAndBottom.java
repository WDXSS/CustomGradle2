 ...
 * 2.1
 */
public class RecycleCustomLayoutManagerScrollTopAndBottom extends RecyclerView.LayoutManager {
	private int mItemWidth;
	private int mItemHeight;
	private SparseArray<Rect> mRectSparseArray = new SparseArray<>();

	public RecycleCustomLayoutManagerScrollTopAndBottom() {
		//需要区分的几个方法
...
//		通过变量mSumDy 保存所有移动过的dy。如果当前移动的距离小于0，就不再累加dy，直接将它移动到y=0的位置，因为之前已经移动的距离是mSumdy。
		...
		for (int i = getChildCount() - 1; i >= 0; i--) {
			...
			if (travel > 0) {
				...
				//向滚动，回收底部超出屏幕以外的 item
				if (getDecoratedTop(childView) - travel > getHeight() - getPaddingBottom()) {
					removeAndRecycleView(childView, recycler);
					continue;
				}
			}
