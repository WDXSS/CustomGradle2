...
public class PorterDuffXfermodeViewXor extends View {
    ...
    public PorterDuffXfermodeViewXor(Context context) {
        this(context, null);
    }

    public PorterDuffXfermodeViewXor(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
//        \alpha_{out} = (1 - \alpha_{dst}) * \alpha_{src} + (1 - \alpha_{src}) * \alpha_{dst}

        //[(1 - Da)*Sa +(1-Sa)*Da , (1 -Dc) * Sc + (1 - Sc) * Dc]
    }

    public PorterDuffXfermodeViewXor(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawText("PorterDuff.Mode.XOR ", 10, heigth * 2, mPaintText);
        //保存状态
