...
public class ExpandableTextView extends TextView {
	...
	private static final boolean SHOW_TO_EXPAND_UNDER_LINE = false;//设置下划线
	private static final boolean SHOW_TO_SHRINK_UNDER_LINE = false;
	...
	private boolean mShowToExpandUnderLine = SHOW_TO_EXPAND_UNDER_LINE;
	private boolean mShowToShrinkUnderLine = SHOW_TO_SHRINK_UNDER_LINE;
	...
	private void initAttr(Context context, AttributeSet attrs) {
		...
		for (int i = 0; i < n; i++) {
			...
			} else if (attr == R.styleable.ExpandableTextView_etv_ToExpandHintUnderLineShow) {
				mShowToExpandUnderLine = a.getBoolean(attr, SHOW_TO_EXPAND_UNDER_LINE);
			} else if (attr == R.styleable.ExpandableTextView_etv_ToShrinkHintUnderLineShow) {
				mShowToShrinkUnderLine = a.getBoolean(attr, SHOW_TO_SHRINK_UNDER_LINE);
			}
		}
		a.recycle();
	}
...
//						span.updateDrawState(tp);
			...
			case STATE_EXPAND: {
				...
				if (mShowToShrinkUnderLine) {
					int shrinkHintLength = mToShrinkHint.length();
					UnderlineSpan span = new UnderlineSpan();
					ssbExpand.setSpan(span, ssbExpand.length() - shrinkHintLength, ssbExpand.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				}
